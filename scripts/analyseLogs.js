// The Server emits log files during its lifetime. Use this script to filter through the log files if you need to find certain events.

const loggers = [] // the names of the loggers (e.g. ['ExampleService']) to retrieve. leave blank to match all
...
const logPath = 'path/to/logs/folder' // it is assumed that there are no other text files on this path other than the log files to analyse
