import * as objects from '@INIT__PATH_ALIAS/shared/util/objects'
...
describe(objects.transformPrimitiveValues, () => {
  const EXPECT_UNUSED = (key: any, value: any) => { throw new InternalError(`Did not expect this function to get called for ${key}: ${value}.` )}
  ...
  test('Replaces shallow primitives', () => {
    ...
    const transformer = (key: any, value: Primitive | undefined) => {
      if (key === 'a' && value === null) {
        ...
        throw new InternalError('Unexpected object type')
      }
    }
