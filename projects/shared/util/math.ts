import { Branded } from '@INIT__PATH_ALIAS/shared/types'
...
function assertConstraint (condition: boolean, value: number, constraint: number, constraintDescription: string): any {
  if (condition) {
    ...
    throw new InternalError(`Expected value ${value} to be ${constraintDescription} ${constraint}`)
  }
}
...
export function clampNorm (value: number, min: number, max: number, centre?: number): Norm {
  if (centre === undefined) {
    ...
    throw new InternalError('Centre must lie between min and max')
  }
