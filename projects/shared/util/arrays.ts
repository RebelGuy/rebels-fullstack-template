import { GenericObject, NumberOnly, PrimitiveKeys, SafeOmit, UnionToIntersection } from '@INIT__PATH_ALIAS/shared/types'
import { InternalError } from '@INIT__PATH_ALIAS/shared/util/error'
import { IDENTITY, assertUnreachable } from '@INIT__PATH_ALIAS/shared/util/typescript'
...
export function single<T> (array: T[]): T {
  if (array.length === 1) {
    ...
    throw new InternalError(`Expected 1 element in the array but found ${array.length}`)
  }
}
...
export function singleOrNull<T> (array: T[]): T | null {
  if (array.length === 0) {
    ...
    throw new InternalError(`Expected 0 or 1 elements in the array but found ${array.length}`)
  }
}

export function first<T> (array: T[]): T {
  if (array.length < 1) {
    throw new InternalError(`Expected at least 1 element in the array but found none`)
  } else {
    return array[0]
  }
}
...
export function zip<T extends GenericObject, U extends GenericObject> (first: T[], second: U[]): (T & U)[] {
  if (first.length !== second.length) {
    throw new InternalError('Cannot zip arrays with different lengths')
  }
...
export function zipOn<T extends GenericObject, U extends GenericObject, Key extends (string | number | symbol) & PrimitiveKeys<T> & PrimitiveKeys<U>> (first: T[], second: U[], key: Key): (Pick<T & U, Key> & Partial<SafeOmit<T & U, Key>>)[] {
  ...
  for (const x of first) {
    const k = x[key]
    if (firstMap.has(k)) {
      throw new InternalError('The key should be unique within the first array')
    } else {
      const copy = { ...x }
      delete copy[key]
      firstMap.set(k, copy)
  ...
  for (const y of second) {
    const k = y[key]
    if (secondMap.has(k)) {
      throw new InternalError('The key should be unique within the second array')
    } else {
      const copy = { ...y }
      delete copy[key]
...
export function zipOnStrict<T extends GenericObject, U extends GenericObject, Key extends (string | number | symbol) & PrimitiveKeys<T> & PrimitiveKeys<U>> (firstArray: T[], secondArray: U[], firstKey: Key, secondKey?: Key, newKey?: Key): (T & U)[] {
  ...
  if (firstArray.length !== secondArray.length) {
    throw new InternalError('Cannot strict-zip arrays with different lengths')
  }
  ...
  if (firstKeys.length !== secondKeys.length || firstKeys.length !== firstArray.length) {
    throw new InternalError('Cannot strict-zip arrays with non-unique keys')
  }

  if (firstKeys.find(x => x == null) || secondKeys.find(y => y == null)) {
    throw new InternalError('Cannot strict-zip arrays when at least one element is null')
  }
  ...
  if (firstKeys.length !== groupedKeys.length) {
    throw new InternalError('Cannot strict-zip arrays with differing keys')
  }
...
> (firstArray: T[], key: Key, ...arrays: Args): UnionToIntersection<T | UnwrapArray<Args[number]>>[] {
  ...
  for (let i = 0; i < arrays.length; i++) {
    try {
      ...
      throw new InternalError(`Unable to zip additional array at index ${i} onto the existing result: ${e.message}`)
    }
  }
  return result
}
