import TimerHelpers, { TimerOptions } from '@INIT__PATH_ALIAS/server/helpers/TimerHelpers'
import { nameof, promised } from '@INIT__PATH_ALIAS/shared/testUtils'
...
describe(nameof(TimerHelpers, 'createRepeatingTimer'), () => {
  ...
  test(`dynamic timer throws if callback doesn't return number`, async () => {
    ...
    await expect(() => timerHelpers.createRepeatingTimer(options, true)).rejects.toThrowError(InternalError)
  })
})
