import { NoNever, Primitive } from '@INIT__PATH_ALIAS/shared/types'
import dotenv from 'dotenv'
import { toConstCase } from '@INIT__PATH_ALIAS/shared/util/text'
...
type EnvironmentVariables = {
  ...
  // myLocalVariable: LocalVariable<false>
  // myOptionalLocalVariable: OptionalLocalVariable<boolean, false>
...
const allCustomEnvVariables: { [K in keyof EnvironmentVariables]: readonly ValueType<K>[] | null } = {
  databaseUrl: null,
...
const allKeys = Object.keys(allCustomEnvVariables) as (keyof EnvironmentVariables)[]
...
function assertValidValue<V extends keyof EnvironmentVariables> (variable: V, value: ValueType<V>): void {
  const validValues = allCustomEnvVariables[variable]
  if (validValues == null) {
    return
