...
import { startTestDb, DB_TEST_TIMEOUT, stopTestDb } from '@INIT__PATH_ALIAS/server/_test/db'
...
const tasks: Task[] = [
  { id: 1, intervalMs: 123, taskType: 'task1' },
  { id: 2, intervalMs: 456, taskType: 'task2' }
]
...
export default () => {
  ...
  describe(nameof(TaskStore, 'getTask'), () => {
    test('Returns the specified task', async () => {
      const result = await taskStore.getTask(tasks[0].taskType as TaskType)
  ...
  describe(nameof(TaskStore, 'updateTask'), () => {
    test('Updates the interval of the specified task', async () => {
      await taskStore.updateTask(tasks[1].taskType as TaskType, 999)
  ...
  describe(nameof(TaskStore, 'getLastExecutionTime'), () => {
    test('Returns the timestamp of the last execution', async () => {
      ...
      const result = await taskStore.getLastExecutionTime(tasks[0].taskType as TaskType)
  ...
  describe(nameof(TaskStore, 'getLastSuccessTime'), () => {
    test('Returns the timestamp of the last successful execution', async () => {
      ...
      const result = await taskStore.getLastSuccessTime(tasks[0].taskType as TaskType)
  ...
  describe(nameof(TaskStore, 'getLastFailureTime'), () => {
    test('Returns the timestamp of the last failed execution', async () => {
      ...
      const result = await taskStore.getLastFailureTime(tasks[0].taskType as TaskType)
  ...
  describe(nameof(TaskStore, 'getTaskLogs'), () => {
    test('Returns the task logs of the specified task', async () => {
      ...
      const result = await taskStore.getTaskLogs(tasks[0].taskType as TaskType)
  ...
  describe(nameof(TaskStore, 'startTask'), () => {
    test('Adds a new entry to the db', async () => {
      ...
      const result = await taskStore.startTask(tasks[0].taskType as TaskType)
