import { ApiError, ApiResponse, PublicObject, ResponseData } from '@INIT__PATH_ALIAS/api-models/types'
import { Primitive } from '@INIT__PATH_ALIAS/shared/types'
import { InternalError } from '@INIT__PATH_ALIAS/shared/util/error'
import { isPrimitive, NO_OP } from '@INIT__PATH_ALIAS/shared/util/typescript'
import LoginContext, { LoginContextType } from '@INIT__PATH_ALIAS/studio/contexts/default/LoginContext'
import RequestContext, { RequestContextType } from '@INIT__PATH_ALIAS/studio/contexts/default/RequestContext'
import useRequireLogin from '@INIT__PATH_ALIAS/studio/hooks/default/useRequireLogin'
import { LOGIN_PATH } from '@INIT__PATH_ALIAS/studio/utility/default/api'
...
> (request: Request<TResponse, TRequestData>, options?: RequestOptions<TResponseData>): RequestResult<TResponseData> {
  ...
  const [cacheKey] = useState(useCache ? `${request.method}-${request.path}-${request.requiresLogin ?? true}` : `${request.method}-${request.path}-${thisRequestCounter}`)
  ...
  if (loginContext == null) {
    if (request.requiresLogin === true && options?.loginToken == null) {
      throw new InternalError('`userRequest` cannot use LoginContext because it is null and no `loginToken` has been provided')
    }
  ...
  const makeRequest = async (type: RequestType): Promise<TriggerResult<TResponseData>> => {
    ...
    try {
      ...
      if (requiresLogin && loginToken == null) {
        throw new InternalError('You must be logged in to do that.')
      }
  ...
  }, [path, method, requiresLogin, loginToken, updateKey, skipLoadOnMount, blockAutoRequest, onDemand, ...objToArr(requestData ?? {})])
